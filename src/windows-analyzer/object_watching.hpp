...
namespace sogen
{
    ...
    emulator_hook* watch_object(windows_emulator& emu, const std::set<std::string, std::less<>>& modules, emulator_object<T> object,
                                const auto verbose, Callback&& on_access, analysis_hook_profile* profile = nullptr,
                                std::shared_ptr<object_watching_state> shared_state = std::make_unique<object_watching_state>())
    {
        const reflect_type_info<T> info{};

        return emu.emu().hook_memory_read(object.value(), static_cast<size_t>(object.size()),
                                          [i = std::move(info), object, &emu, verbose, modules, state = std::move(shared_state),
                                           on_access = std::forward<Callback>(on_access),
                                           profile](cpu_interface& cpu, const uint64_t address, const void*, const size_t size) {
                                              ...
                                              emu.dispatch_on_cpu(cpu, [&] {
                                                  ...
                                                  const auto is_main_access =
                                                      !mod || (mod == emu.mod_manager.executable || modules.contains(mod->name));
    ...
    {
        return watch_object<T>(emu, modules, emulator_object<T>{emu.emu(), address}, verbose, std::forward<Callback>(on_access), profile,
                               std::move(state));
    }
