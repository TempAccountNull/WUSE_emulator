...
pub(crate) struct RuntimeFunctions {
    ...
    pub exec_write_notify: FuncId,
    pub noop_barrier: FuncId,
...
pub struct JIT {
    ...
    /// Opt-in cumulative compile costs; read only while the VM is parked.
    pub profile_enabled: bool,
    pub profile_compile_calls: u64,
    ...
    /// Guest entry points and their owning generated function for each compilation group.
    compiled: Vec<Vec<(u64, JitFunction)>>,
...
impl JIT {
    pub fn new(cpu: &icicle_cpu::Cpu) -> Self {
        ...
        translator_ctx.exec_write_wake = std::env::var("SOGEN_SMP_EXEC_WRITE_WAKE").as_deref() == Ok("1");
    ...
    /// Lower only the registered empty instruction hook through a direct opaque call.
    /// Other hooks retain the normal callback and exception path.
    pub fn set_noop_barrier_hook(&mut self, id: pcode::HookId) {
        self.translator_ctx.noop_barrier_hook = Some(id);
    }
    ...
    /// Sum generated host code retained by this JIT module, including invalidated functions.
    /// Query only while the owning VM is parked.
    pub fn generated_code_bytes(&self) -> u64 {
    ...
    pub fn invalidate(&mut self, block_id: usize) {
        if let Some(owners) = self.block_mapping.get(&block_id) {
            for &id in owners {
                for &(addr, function) in &self.compiled[id] {
                    if self.entry_points.get(&addr).is_some_and(|current| std::ptr::fn_addr_eq(*current, function)) {
                        let key = Self::lookup_key(addr);
                        if self.active[key].0 == addr && std::ptr::fn_addr_eq(self.active[key].1, function) {
                            self.active[key] = INITIAL_LOOKUP_TABLE_VALUE;
                        }
    ...
    pub fn compile(&mut self, target: &CompilationTarget) -> ModuleResult<()> {
        ...
        let result = self.compile_impl(target);
        self.profile_compile_calls += 1;
        self.profile_compile_nanos = self.profile_compile_nanos.saturating_add(
            start.elapsed().as_nanos().min(u64::MAX as u128) as u64,
        );
        result
    }

    fn compile_impl(&mut self, target: &CompilationTarget) -> ModuleResult<()> {
        ...
        self.compiled.push(target.entry_points().map(|addr| (addr, self.entry_points[&addr])).collect());

        for &block_id in target.targets {
            self.block_mapping.entry(block_id).or_default().push(group_id);
        }
    ...
    fn translate_and_define(
        ...
        self.generated_code_bytes = self.generated_code_bytes.saturating_add(u64::from(size));
...
fn init_module(endianness: Endianness) -> (JITModule, RuntimeFunctions) {
    ...
    define_fn_symbol!(runtime::exec_write_notify);
    define_fn_symbol!(runtime::noop_barrier);
...
mod historical_owner_tests {
    use super::{CompilationTarget, JIT};
    use icicle_cpu::{lifter, Arch, Cpu};

    #[test]
    fn invalidating_block_revokes_older_superblock_owner() {
        let cpu = Cpu::new_boxed(Arch::none());
        let mut jit = JIT::new(&cpu);
        let a = cpu.arch.sleigh.get_varnode("a").unwrap().slice(0, 4);
        let b = cpu.arch.sleigh.get_varnode("b").unwrap().slice(0, 4);
        let out = cpu.arch.sleigh.get_varnode("c").unwrap().slice(0, 4);
        let make_block = |entry: u64, next: u64| {
            let mut pcode = pcode::Block::new();
            pcode.push(pcode::Instruction::from((out, pcode::Op::IntAdd, a, b)));
            lifter::Block {
                pcode,
                entry: Some(entry),
                start: entry,
                ...
                context: 0,
                exit: lifter::BlockExit::Jump {
                    target: lifter::Target::External(next.into()),
                },
                breakpoints: 0,
                num_instructions: 0,
            }
        ...
        const B: u64 = 0x2000;
        const C: u64 = 0x3000;
        let blocks = [make_block(A, B), make_block(B, 0x4000), make_block(C, 0x4000)];

        // A and B originally share generated code. B is then independently recompiled,
        // so the current B entry no longer points to the older A+B function.
        jit.compile(&CompilationTarget::new(&blocks, &[0, 1])).unwrap();
        let older = jit.entry_points[&A];
        assert!(std::ptr::fn_addr_eq(older, jit.entry_points[&B]));
        jit.compile(&CompilationTarget::new(&blocks, &[1])).unwrap();
        assert!(!std::ptr::fn_addr_eq(older, jit.entry_points[&B]));
        jit.compile(&CompilationTarget::new(&blocks, &[2])).unwrap();
        assert!(jit.lookup_fast(A).is_some());
        assert!(jit.lookup_fast(B).is_some());
        assert!(jit.lookup_fast(C).is_some());

        let prior_dead = jit.dead;
        jit.invalidate(1);
        assert!(!jit.entry_points.contains_key(&A), "older A entry reaches stale B code");
        assert!(!jit.entry_points.contains_key(&B));
        assert!(jit.entry_points.contains_key(&C));
        assert!(jit.lookup_fast(A).is_none());
        assert!(jit.lookup_fast(B).is_none());
        assert!(jit.lookup_fast(C).is_some());
        assert_eq!(jit.dead, prior_dead + 2);
        jit.invalidate(1);
        assert_eq!(jit.dead, prior_dead + 2, "invalidation must be idempotent");
        unsafe { jit.reset() };
    }
}
...
fn declare_runtime_functions(module: &mut JITModule) -> ModuleResult<RuntimeFunctions> {
    ...
    Ok(RuntimeFunctions {
        ...
        exec_write_notify: import_fn!("runtime::exec_write_notify", (I64, I64, I64) -> ()),
        noop_barrier: import_fn!("runtime::noop_barrier", (I64) -> ()),
