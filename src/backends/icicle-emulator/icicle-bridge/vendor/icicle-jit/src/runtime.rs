use icicle_cpu::{Cpu, ExceptionCode, InternalError, mem::{perm, physical::PageData}};

/// Called only after a JIT direct store to a shared page already marked as translated.
/// The pointer is the stable PageData allocation behind the write TLB entry.
/// An opaque call for the lean instruction barrier. Passing Cpu* tells the JIT that
/// runtime code may observe or change VM state; the state is flushed before this call.
#[inline(never)]
pub extern "C" fn noop_barrier(cpu: *mut Cpu) {
    std::hint::black_box(cpu);
    std::sync::atomic::compiler_fence(std::sync::atomic::Ordering::SeqCst);
}

pub extern "C" fn exec_write_notify(page: *const PageData, addr: u64, len: u64) {
    unsafe { (*page).notify_exec_write_jit(addr, len as usize); }
}
