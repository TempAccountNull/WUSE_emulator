...
impl<'a, 'b> Ctx<'a, 'b> {
    ...
    /// Preserve the normal hook's varnode fence and PC store, then call a direct opaque
    /// runtime thunk. Only the bridge's proven-empty lean hook may use this path.
    pub fn call_noop_barrier(&mut self) {
        self.trans.varnode_fence();
        let current_pc = self.trans.builder.ins().iconst(types::I64, self.trans.last_addr as i64);
        let reg_pc = self.trans.ctx.reg_pc;
        let pc_sized = self.trans.resize_int(current_pc, 8, reg_pc.size);
        self.trans.vm_ptr.store_var(&mut self.trans.builder, reg_pc, pc_sized);
        self.trans.builder.ins().call(self.trans.symbols.noop_barrier, &[self.trans.vm_ptr.0]);
    }
