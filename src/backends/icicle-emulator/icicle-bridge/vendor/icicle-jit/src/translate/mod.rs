...
struct Symbols {
    ...
    exec_write_notify: FuncRef,
    noop_barrier: FuncRef,
}
...
impl Symbols {
    fn import(
        ...
        Self {
            ...
            exec_write_notify: module.declare_func_in_func(funcs.exec_write_notify, current),
            noop_barrier: module.declare_func_in_func(funcs.noop_barrier, current),
        }
    }
}
...
pub(crate) struct TranslatorCtx {
    ...
    /// Opt-in code-page JIT store notification. Default-generated code is unchanged.
    pub exec_write_wake: bool,
    pub noop_barrier_hook: Option<pcode::HookId>,
    page_size: u64,
...
impl TranslatorCtx {
    pub(crate) fn new(arch: &Arch) -> Self {
        ...
        Self {
            ...
            exec_write_wake: false,
            noop_barrier_hook: None,
    ...
    fn translate_block(&mut self, block_id: usize, block: &IcicleBlock) {
        ...
        for (i, stmt) in block.pcode.instructions.iter().enumerate() {
            ...
            match stmt.op {
                ...
                Op::Hook(id) => {
                    if ctx.trans.ctx.noop_barrier_hook == Some(id) {
                        ctx.call_noop_barrier();
                    } else {
                        ctx.call_hook(id);
                    }
                    // A prior interpreter fallback (for example, a dynamic register
                    // load/store) can set Cpu.exception. The empty barrier must still
                    // observe it before the next guest instruction executes.
