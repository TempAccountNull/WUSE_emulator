...
pub(super) fn store_ram(trans: &mut Translator, guest_addr: pcode::Value, value: pcode::Value) {
    ...
    if let Some(host_addr) = host_addr {
        ...
        let page_offset = trans.builder.ins().band_imm(guest_addr_val, (1_i64 << OFFSET_BITS) - 1);
        let page_base = trans.builder.ins().isub(host_addr, page_offset);
        let code_seen_addr = trans.builder.ins().iadd_imm(page_base, offset_of!(PageData, smp_code_seen) as i64);
        let code_seen = trans.builder.ins().atomic_load(types::I8, MemFlags::trusted(), code_seen_addr);
        trans.branch_zero(code_seen, success_block);
        let epoch_addr = trans.builder.ins().iadd_imm(page_base, offset_of!(PageData, code_epoch) as i64);
        let one = trans.builder.ins().iconst(types::I64, 1);
        trans.builder.ins().atomic_rmw(types::I64, MemFlags::trusted(), AtomicRmwOp::Add, epoch_addr, one);
        if trans.ctx.exec_write_wake {
            let size = trans.builder.ins().iconst(types::I64, store_size as i64);
            trans.builder.ins().call(trans.symbols.exec_write_notify, &[page_base, guest_addr_val, size]);
        }
