...
fn decode(cpu: &Cpu, bytes: &[u8], next: u64) -> Result<Move, Exception> {
    ...
    if mode == 1 {
        ...
        offset += 1;
        address = address
            .wrapping_add((displacement * if family == 2 { width as i64 } else { 1 }) as u64);
    } else if displacement32 {
        let value = bytes.get(offset..offset + 4).ok_or_else(invalid)?;
        offset += 4;
        address = address.wrapping_add(i32::from_le_bytes(value.try_into().unwrap()) as i64 as u64);
    }
    if offset != bytes.len() {
        return Err(invalid());
    }
